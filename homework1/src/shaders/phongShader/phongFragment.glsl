...
float findBlocker( sampler2D shadowMap,  vec2 uv, float zReceiver ) {
  ...
  // bigger stride for softer shadow 
  float filterStride = 10.0;
  float filterRange = 1.0 / shadowMapSize * filterStride;

  // points within shadow
  int count = 0;
  ...
  if(count == BLOCKER_SEARCH_NUM_SAMPLES)
    return 10.0;
  // avg
  return blockDepth / float(count);
}
...
float PCF(sampler2D shadowMap, vec4 coords) {
  ...
  return float(count) / float(PCF_NUM_SAMPLES);
}
...
float PCSS(sampler2D shadowMap, vec4 coords){
  ...
  float shadowMapSize = 400.0;
  float filterStride = 5.0;
  float filterRange = 1.0 / shadowMapSize * filterStride * wPenumbra;
